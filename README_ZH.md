...
> **如果打破25%等权——不同股票引擎的最优权重是多少？**
> 
> 本项目分三阶段回答了这些问题：Phase 1 验证了 **12组资产组合** 的25%等权稳健性（~40,000个入场时点，2003-2026）；Phase 2 确认了持有24月胜率>90%；Phase 3 打破等权，对四个版本进行 **341组合网格搜索** 探索最优权重。

> **📄 研究论文：** [reports/研究论文.md](reports/研究论文.md)（Phase 1/2，含10篇学术参考文献）
> **📄 权重优化论文：** [reports/最优权重分析报告.md](reports/最优权重分析报告.md)（Phase 3，8张图表，341组合网格搜索）
...
## 📊 第四阶段：蒙特卡洛模拟 — 34年DCA前瞻

**核心问题：用户变体（红利低波30%+美股QDII 30%+资源能源20%+黄金12%+债券+现金8%）通过渐进加码定投（2000→4000/月封顶），在34年后（26→60岁）能否达到1000万目标？**

| 场景 | 组合年化 | 60岁中位数 | 1000万达成率 |
|:---:|:--------:|:---------:|:----------:|
| 保守（均值-0.3σ） | 5.0% | 351万 | 0.1% |
| **中位（合理预期）** | **6.5%** | **493万** | **2.3%** |
| 乐观（均值+0.3σ） | 8.2% | 701万 | 16.3% |

### 关键发现

1. **中位场景60岁中位数493万** — 仅达目标49%，在现有定投方案下1000万几乎不可达
2. **所有场景尾部安全** — 最差5%仍有210-398万，从未亏损本金
3. **最后10年贡献总资产45%+** — 50岁后复利加速，放弃是最可惜的
4. **储蓄率 > 回报率** — 月投从4k→8k可使终值翻倍至986万，而提高回报率靠运气

### 新旧配比对比（用户变体 vs 经典25%×4）

| 指标 | 原配比（旧） | 新配比（25%×4）（用户建议） |
|:----|:----------:|:------------------------:|
| 组合年化（中位） | **6.5%** | **5.4%** |
| 月波动率 | 2.43% | **1.63%** |
| 60岁中位数 | **493万** | 408万 |
...
> 新配比更稳但终值更低。原配比用可接受的波动增量换取了显著更高的终值预期。

完整报告：
- `蒙特卡洛模拟/蒙特卡洛模拟分析报告.md` — 34年前瞻分析
- `蒙特卡洛模拟/新旧配比对比报告.md` — 两组配比全面对比
- `蒙特卡洛模拟/charts/01_年龄_资产曲线.png` — 三场景年龄资产曲线
